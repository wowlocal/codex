...
fn config_layer_stack(requirements_toml: &str) -> ConfigLayerStack {
    config_layer_stack_with_user_config(requirements_toml, None)
}
...
"#,
    ...
    for source in [
        ...
        assert_eq!(validate_source(&stack, &parse_source(source, None)), Ok(()));
    }
    assert!(
        validate_source(
            &stack,
            &parse_source("https://github.com/example/plugins.git", None),
        )
        .is_err()
    );
}
...
"#
    ...
    assert_eq!(
        validate_source(
            &stack,
            &parse_source(allowed.to_string_lossy().as_ref(), None),
        ),
        Ok(())
    );
    assert!(
        validate_source(
            &stack,
            &parse_source(denied.to_string_lossy().as_ref(), None),
        )
        .is_err()
    );
}
...
"#,
    ...
    let source = parse_source(bundled_root.to_string_lossy().as_ref(), None);
