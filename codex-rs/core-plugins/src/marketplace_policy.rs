...
fn compile_allowed_source(
    ...
    match allowed_source {
        MarketplaceAllowedSourceToml::GitUrl { url, ref_name } => {
            let source = parse_marketplace_source(url, ref_name.clone()).map_err(|err| {
                format!("invalid marketplace allowed source `{key}` in {requirement_source}: {err}")
            })?;
            let MarketplaceSource::Git { url, ref_name } = source else {
                return Err(format!(
                    "invalid marketplace allowed source `{key}` in {requirement_source}: expected a Git URL"
                ));
            };
            Ok(AllowedMarketplaceSource::GitUrl { url, ref_name })
        }
        MarketplaceAllowedSourceToml::GitHostPattern { host_pattern } => Regex::new(host_pattern)
            .map(AllowedMarketplaceSource::GitHostPattern)
            .map_err(|err| {
                format!("invalid marketplace allowed source `{key}` in {requirement_source}: {err}")
            }),
        MarketplaceAllowedSourceToml::Local { path } => {
            Ok(AllowedMarketplaceSource::Local(path.clone()))
        }
    }
}
...
fn managed_local_marketplace_name(codex_home: &Path, root: &Path) -> Option<&'static str> {
    ...
    let runtime_root = home_dir()?
        .join(".cache/codex-runtimes/codex-primary-runtime/plugins")
