...
struct SlowInventoryServer {
    tool_name: Arc<String>,
    inventory_call_count: Arc<AtomicUsize>,
}
...
impl ServerHandler for SlowInventoryServer {
    ...
    async fn list_resources(
        ...
        self.inventory_call_count.fetch_add(1, Ordering::SeqCst);
...
async fn mcp_server_status_list_tools_and_auth_only_skips_slow_inventory_calls() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let (mcp_server_url, mcp_server_handle, inventory_call_count) =
...
"#
    ...
    assert_eq!(inventory_call_count.load(Ordering::SeqCst), 0);
...
async fn start_slow_inventory_mcp_server(
    ...
    let addr = listener.local_addr()?;
    let tool_name = Arc::new(tool_name.to_string());
    let inventory_call_count = Arc::new(AtomicUsize::new(0));
    let inventory_call_count_for_service = Arc::clone(&inventory_call_count);
    let mcp_service = StreamableHttpService::new(
        move || {
            Ok(SlowInventoryServer {
                tool_name: Arc::clone(&tool_name),
                inventory_call_count: Arc::clone(&inventory_call_count_for_service),
            })
        },
        Arc::new(LocalSessionManager::default()),
        StreamableHttpServerConfig::default(),
    ...
    Ok((format!("http://{addr}"), handle, inventory_call_count))
}
