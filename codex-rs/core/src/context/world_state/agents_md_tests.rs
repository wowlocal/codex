...
fn renders_full_state_and_omits_unchanged_state() {
    ...
    assert_eq!(
        ...
        render_fragments(state.render_diff(&state))
    );
}
...
fn changed_and_removed_state_supersedes_previous_instructions() {
    ...
    assert_eq!(
        ...
        render_fragments(removed.render_diff(&current)),
    );
}

#[test]
fn unknown_historical_state_is_always_superseded() {
    let mut previous = WorldState::default();
    previous.add_section(AgentsMdState::unknown());
    ...
    let mut absent = WorldState::default();
    absent.add_section(AgentsMdState::default());
    assert_eq!(
        vec![user_message(
            "# AGENTS.md instructions\n\n<INSTRUCTIONS>\nThe previously provided AGENTS.md instructions no longer apply.\n</INSTRUCTIONS>",
        )],
        render_fragments(absent.render_diff(&previous)),
    );
}
